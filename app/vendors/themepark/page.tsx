...
const ThemePark: React.FC = () => {
  ...
  const [Themeparks, setThemeparks] = useState<Freelancer[]>([]);
  const [isLoading, setIsLoading] = useState<boolean>(false);
  const [error, setError] = useState<string | null>(null);
  const [openModal, setOpenModal] = useState<string | null>(null);
  const [currentPage, setCurrentPage] = useState<number>(1);
  const [selectedThemeParkId, setSelectedThemeParkId] = useState<string | null>(null);
  ...
  const fetchThemeparks = async () => {
    ...
    setError(null);
    try {
      const res = await axiosInstance.get('/themeparks');
      if (res?.data) {
        setThemeparks(res.data);
      }
    } catch (error) {
  ...
  const handleDeleteClick = (id: any) => {
    setSelectedThemeParkId(id);
  };

  const handleDeleteCancel = () => {
    setSelectedThemeParkId(null);
  };

  const confirmDeleteHandler = async () => {
    if (selectedThemeParkId) {
      try {
        await axiosInstance.delete(`/themeparks/${selectedThemeParkId}`);
        setThemeparks(prev => prev.filter(f => f.id !== selectedThemeParkId));
        toast({
          variant: "default",
          title: "Success!",
  ...
  const totalPages = Math.ceil(Themeparks?.length / itemsPerPage);
  const currentThemeparks = Themeparks.slice((currentPage - 1) * itemsPerPage, currentPage * itemsPerPage);
      ...
      <Card className="w-[98%]">
        <CardHeader>
          <div className='w-full h-auto flex justify-between items-center'>
            <div>
              <CardTitle className="text-lg md:text-2xl font-semibold">Themeparks</CardTitle>
              <CardDescription className='text-xs mt-1'>Overall {Themeparks?.length}  Themepark&apos;s</CardDescription>
            </div>
            <div>
              <Link href='/vendors/themepark/add'>
                <Button>
                  <Plus className="mr-2 h-4 w-4" /> Add ThemePark
                </Button>
              </Link>
            </div>
          </div>
        ...
        <CardContent>
          {isLoading ? (
            ...
            <Table>
              ...
              <TableBody>
                {currentThemeparks.map(item => (
                  <TableRow key={item?.id}>
                    <TableCell className='font-semibold'>{(currentPage - 1) * itemsPerPage + currentThemeparks.indexOf(item) + 1}</TableCell>
                    <TableCell className='font-semibold'>{item?.themeparkname}</TableCell>
                      ...
                      <Dialog>
                        ...
                        {openModal === item?.id && (
                          <DialogContent className="sm:max-w-lg">
                            <DialogHeader>
                              <DialogTitle>Plans for {item?.themeparkname}</DialogTitle>
                              <DialogDescription>
                    ...
                    <TableCell className='text-end'>
                      <Link href={`/vendors/themepark/edit/${item?.id}`}>
                        <Button variant="outline" size="icon" className='me-3'>
                          <Pencil className='h-4 w-4' />
                        </Button>
                      </Link>
                      <AlertDialog open={!!selectedThemeParkId} onOpenChange={(open) => {
                      ...
                      <Link href={`/Themeparks/view/${item?.id}`}>
                        <Button variant="outline" size="icon" className='me-3'>
